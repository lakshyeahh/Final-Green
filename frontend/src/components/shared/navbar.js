...
const NavigationMenuDemo = ({ userData}) => {
  ...
  return (
    ...
    <NavigationMenu.Root className=" NavigationMenuRoot "  >
      <Flex gap='20px' className='pt-4' style={{ justifyContent: 'center', alignItems: 'center', width: '100%' }}>
        <div className='logo-container' style={{ display: 'flex', alignItems: 'center' }}>
          <img src={Logo} alt="Logo" className="logo-image "  />
          <div className='logo-text ' style={{fontFamily: '"Abril Fatface", serif', fontWeight: '400 ', fontSize: '40px'}}>Green</div>
        </div>
  ...
  <ListItem title="Forum" href="/forum">
    Join discussions and share your thoughts.
        ...
        {!userData && <Link to="/login">
          <Button className='login-button' variant='solid'color='mint' highContrast='true' >Log In</Button>
        </Link>}
        

      {userData && (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <p style={{ marginRight: '20px' }}>🙋 {userData.name}</p>
